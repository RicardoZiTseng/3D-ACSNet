...
def deploy(original_model, deploy_model, weights_path, fused_weights_path, eps=1e-5):
    ...
    conv_nxnxn_var_names = [name for name in layer_names if CUBE_KERNEL_KEYWORD in name and '.conv_pass' not in name]
    for conv_nxnxn_name in conv_nxnxn_var_names:
        # print(conv_nxnxn_name)
        conv_nxnxn_kernel, bias = original_model.get_layer(conv_nxnxn_name).get_weights()
        ...
        fused_bias = betas[0] + betas[1] + betas[2] + betas[3] - (means[0] - bias) * gammas[0] / stds[0] - means[1] * gammas[1] / stds[1] \
    ...
    for name in layer_names:
        if '_nxnxn' not in name and '_1xnxn' not in name and '_nx1xn' not in name and '_nxnx1' not in name and '.add' not in name:
            # print(name)
            deploy_model.get_layer(name).set_weights(original_model.get_layer(name).get_weights())

    for name in layer_names:
        if '.conv_pass' in name:
            # print(name)
            deploy_model.get_layer(name).set_weights(original_model.get_layer(name).get_weights())
