...
from keras.layers import (BatchNormalization, Conv3D, Conv3DTranspose, Dropout, SpatialDropout3D, MaxPooling3D,
                          Input, Lambda, Layer, ReLU, Reshape, Softmax,
                          concatenate, merge)
from keras.losses import categorical_crossentropy
from .ac_layer import ACBlock
...
class DenseUNet(object):
    ...
    def build_model(self):
        ...
        model = keras.Model(inputs=self.images, outputs=seg, name='deploy' if self.deploy else 'original')
        return model
    ...
    def dense_layer(self, f_in, name):
        ...
        x = self._conv(x, self.growth_rate, 1, name=name+'.aconv', deploy=self.deploy)
    ...
    def _conv(self, x_in, out_channel, stride=1, use_bias=True, name=None, deploy=False, use_same_padding=False):
        x = ACBlock(x_in, name+'.acb', out_channel, self.conv_size, stride, use_same_padding,
                kernel_initializer=keras.initializers.he_normal(),
    ...
    def transition_down(self, f_in, name='trans_down'):
        ...
        x = self._conv(f_in, int(channels * self.compress_rate), 1, name=name+'.conv0', deploy=self.deploy)
        x = self._conv(x, x.get_shape()[-1].value, 2, name=name+'.conv_pass', deploy=False, use_same_padding=True)
        return x
